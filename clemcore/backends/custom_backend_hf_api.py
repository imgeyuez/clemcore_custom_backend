...
class HuggingfaceLocalModel(backends.BatchGenerativeModel):
    ...
    """
    Core batch response implementation with speaker-listener branching.
    ...
    # Render prompts for each conversation
    ...
    # ensure list type
    if isinstance(rendered_chats, str):
        rendered_chats = [rendered_chats]

    results = []

    for chat_text, messages in zip(rendered_chats, batch_messages):
        # ---- Decide role based on prompt
        if '\"Answer\"' in chat_text:
            is_listener_turn = True
        else:
            is_listener_turn = False # is speaker
        ...
        if is_listener_turn:
            ...
            prompt_info = {"inputs": chat_text, "role": "listener"}
            response_obj = {"response": decoded[0]}
            response_text = decoded[0]

            results.append((prompt_info, response_obj, response_text))
            ...
            gen_args = {
                ...
                "top_p": getattr(self.model.generation_config, "top_p", 0.9),
                "max_new_tokens": self.max_tokens,
                "attention_mask": encoding["attention_mask"],
            ...
            speaker_output_ids = self.model.generate(encoding["input_ids"], **gen_args)
            speaker_outputs = self.tokenizer.batch_decode(speaker_output_ids, skip_special_tokens=True)

            # 2) Compute speaker logprobs
            speaker_logps = []
            for out_ids in speaker_output_ids:
                labels = out_ids.unsqueeze(0)
                with torch.no_grad():
                    outputs = self.model(labels, labels=labels)
                    logp = -outputs.loss.item() * labels.size(1)
                speaker_logps.append(logp)

            # 3) Listener scoring for each candidate
            listener_logps = []
            for candidate in speaker_outputs:
                listener_prompt = self._make_listener_prompt(messages, candidate)
                listener_enc = self.tokenizer(listener_prompt, return_tensors="pt", padding=True).to(self.device)

                with torch.no_grad():
                    outputs = self.model(**listener_enc, labels=listener_enc["input_ids"])
                    logp = -outputs.loss.item() * listener_enc["input_ids"].size(1)
                listener_logps.append(logp)

            # 4) Combine scores
            final_scores = [lambda_s * ps + lambda_l * pl for ps, pl in zip(speaker_logps, listener_logps)]

            # 5) Pick best candidate
            best_idx = int(torch.tensor(final_scores).argmax())
            best_response = speaker_outputs[best_idx]

            # 6) Package
            prompt_info = {"inputs": chat_text, "role": "speaker", "num_samples": num_samples}
            response_obj = {
                "candidates": speaker_outputs,
                "speaker_logps": speaker_logps,
                "listener_logps": listener_logps,
...
def _make_listener_prompt(self, original_messages: List[Dict], candidate: str) -> str:
    """
    Build listener-style prompt from original conversation + a candidate speaker utterance.
    """
    listener_messages = original_messages + [{"role": "speaker", "content": candidate}]
    return self.tokenizer.apply_chat_template(listener_messages, add_generation_prompt=True, tokenize=False)
